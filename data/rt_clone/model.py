...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.causal_mask = (
            config.causal_mask if hasattr(config, "causal_mask") else True
        )
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.test = {"n_recur[cross,uec]": False, "n_layer[uec_last]": False}
    ...
    def forward(self, idx, targets=None, idx_ulb=None):
        ...
        for _ in range(self.n_recur):
            for block in self.blocks:
                x, att_to_check = block(
                    x
                )  # (batch_size, 81, 128) (batch_size, num_heads, 81, 81)
        ...
        if targets is not None:
            ...
            for logit in logits:  # (batch_size, 81, 9)
                NULL_LOSS_WEIGHT = 0.5
                if "asymmetric" in self.losses:
                    total_entropy = F.cross_entropy(logit.reshape(-1, logit.size(-1)), targets.view(-1))
                    null_logit_ratio = logit.reshape(-1, logit.size(-1))[:,-1] / logits.reshape(-1, logit.size(-1)) 

                    #TODO: finish 
    
                    
                    pass #loss += F.cross_entropy(logits.)


                loss += F.cross_entropy(
                    logit.reshape(-1, logit.size(-1)), targets.view(-1)
                )
                ...
                if "c1" in self.losses:
                    ...
                    L_c1 = (
                        reg_cardinality(probs.permute(0, 3, 2, 1).reshape(-1, 9), num=1)
                        + reg_cardinality(
                            probs.permute(0, 3, 1, 2).reshape(-1, 9), num=1
                        )
                        + reg_cardinality(
                            probs.reshape((-1, 3, 3, 3, 3, 9))
                            .permute(0, 5, 1, 3, 2, 4)
                            .reshape(-1, 9),
                            num=1,
                        )
            ...
            for att in atts:  # (batch_size, num_heads, 81, 81) for Sudoku
                if "att_c1" in self.losses:
                    att_p = F.softmax(att, dim=-1).reshape(
                        -1, 81, 81
                    )  # (batch_size * num_heads, 81, 81)
