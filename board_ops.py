...
def encodeBoard(sudoku, puzzl_mat, guess_mat, curs_pos, action_list):
	'''
	Encodes the current board state and encodes the given action(s). 
	If there is only one action, it runs the action
	and then encodes the new board state.
	...
	'''
	nodes,_ = sparse_encoding.sudokuToNodes(puzzl_mat, guess_mat, curs_pos, action_list, None)
	benc,coo,a2a = sparse_encoding.encodeNodes(nodes)

	reward_list = []
	for action_typ,action_val in action_list: 
		if len(action_list) > 1: 
			# no side-effects for any of them. 
			# (this would indicate that the formulation is not quite right-!)
			reward = runAction(sudoku, np.copy(puzzl_mat), np.copy(guess_mat), np.copy(curs_pos), action_typ, action_val)
		else: 
			reward = runAction(sudoku, puzzl_mat, guess_mat, curs_pos, action_typ, action_val)
		reward_list.append(reward)

	nodes,_ = sparse_encoding.sudokuToNodes(puzzl_mat, guess_mat, curs_pos, action_list, reward_list) 
	newbenc,coo,a2a = sparse_encoding.encodeNodes(nodes)

	return benc, newbenc, coo, a2a, reward_list
...
def enumerateBoards(puzzles):
	...
	return orig_board_enc, new_board_enc, coo, a2a, rewards_enc,
