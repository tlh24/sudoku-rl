...
class Trainer:
    def __init__(self, model, train_dataset, train_dataset_ulb, test_dataset, config):
        ...
        train_dataset_1000 = torch.utils.data.Subset(
            train_dataset, list(range(min(1000, len(train_dataset))))
        )
        # dataloaders for evaluation
        self.train_dataloader = DataLoader(
            train_dataset_1000, batch_size=config.batch_size
        )
    ...
    def train(self):
        ...
        def run_epoch(split):
            ...
            if is_train and config.label_size < config.batch_size:
                ...
                pbar = (
                    tqdm(enumerate(self.loader_lb), total=len(self.loader_lb))
                    if is_train
                    else enumerate(self.test_dataloader)
                )
                for it, (x, y) in pbar:
                    # place data on the correct device
                    x = x.to(self.device)
            ...
            if (epoch + 1) % self.eval_interval == 0:
                for eval_func in self.eval_funcs:
                    ...
                    if self.wandb:
                        ...
                        self.wandb.log(
                            {
                                f"test_acc/{eval_func.__name__}_board": 100
                                * correct
                                / total
                            },
                            step=epoch + 1,
                        ...
                        self.wandb.log(
                            {
                                f"test_acc/{eval_func.__name__}_cell": 100
                                * singleCorrect
                                / singleTotal
                            },
                            step=epoch + 1,
                        ...
                        self.wandb.log(
                            {
                                f"train_acc/{eval_func.__name__}_board": 100
                                * correct
                                / total
                            },
                            step=epoch + 1,
                        ...
                        self.wandb.log(
                            {
                                f"train_acc/{eval_func.__name__}_cell": 100
                                * singleCorrect
                                / singleTotal
                            },
                            step=epoch + 1,
                ...
                #             att[l].numpy(),
                #             all_tokens,
                #             filename=f"heatmap/{self.prefix}epoch{epoch+1}_layer{l+1}",
                #         )
                #     visualize_cell_attention(
                #         att[0].numpy(),
                #         indices,
                #         cell_tokens,
                #         filename=f"heatmap/{self.prefix}epoch{epoch+1}_layer1_cell",
                #     )
                #
                #     # visualize positional embedding
                #     pos_emb = (
                #         model.pos_emb.detach().clone().squeeze().cpu()
                #     )  # (81, 128)
                #     pos_emb /= pos_emb.norm(dim=1, keepdim=True)  # (81, 128)
                #     sim = (
                #         torch.matmul(pos_emb, pos_emb.T).unsqueeze(0).numpy()
                #     )  # (81, 81)
                #     all_tokens = range(81)
                #     visualize_token2token_scores(
                #         sim,
