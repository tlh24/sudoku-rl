...
def update_precond_UVd_math_(U, V, d, v, h, step, step_normalizer, tiny):
    ...
    # add in float16 (half) support by casting back to float32
    # (linalg routines are not implmented for float16, by design)
    if IpVtU.dtype == torch.float16: 
        invQtv = invQtv - V.mm(torch.linalg.solve(IpVtU.t().to(torch.float), U.t().mm(invQtv).to(torch.float)).half() )
        invPv  = invQtv - U.mm(torch.linalg.solve(IpVtU.to(torch.float),     V.t().mm(invQtv).to(torch.float)).half() )
    else:
