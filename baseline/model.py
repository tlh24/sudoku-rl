        ...
        if targets is not None:
            ...
            for logit in logits: # (batch_size, 81, 9)
                ...
                if 'c1' in self.losses:
                    ...
                    L_c1 = ste.reg_cardinality(probs.permute(0,3,2,1).reshape(-1,9), num=1) + \
                        ste.reg_cardinality(probs.permute(0,3,1,2).reshape(-1,9), num=1) + \
                        ste.reg_cardinality(probs.reshape((-1,3,3,3,3,9)).permute(0,5,1,3,2,4).reshape(-1,9), num=1)
            ...
            for att in atts: # (batch_size, num_heads, 81, 81) for Sudoku
                if 'att_c1' in self.losses:
                    ...
                    loss += ste.reg_att_sudoku_c1(att_p) * self.hyper[1]
