...
class CausalSelfAttention(nn.Module):
	# it's not actually causal anymore -- tlh
	...
	def forward(self, x):
		...
		if self.flash and not USE_L1A:
			# efficient attention using Flash Attention CUDA kernels
			y = torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=None, dropout_p=self.dropout if self.training else 0, is_causal=False)
			y = y.transpose(1, 2).contiguous().view(B, T, C) # re-assemble all head
		elif USE_L1A: 
			# l1a expects bthd; q,k,v are bhtd
			q = q.transpose(1,2).contiguous()
			k = k.transpose(1,2).contiguous()
			a = self.l1a_f(q, k) # includes sqrt(head)
			# output is b,src,dst,heads
			a = F.softmax(a, 1) # see l1attn.py -- sm over src
			y = torch.einsum('bsdh, bhsw -> bdhw', a, v)
			y = y.contiguous().view(B, T, C) # re-assemble all head
		else:
			# this works!
			a = torch.einsum('bhtd,bhsd -> btsh', q, k) / math.sqrt(q.shape[-1])
			a = F.softmax(a, 2)
			# a = a * msk 
			y = torch.einsum('btsh,bhsd -> bthd', a, v)
			y = y.contiguous().view(B, T, C) # re-assemble all head
...
class GPT(nn.Module):
	...
	def __init__(self, config):
		...
		self.lm_head = nn.Linear(config.n_embd, config.in_size, bias=True)
		...
		print("NanoGPT: number of parameters: %.2fM" % (self.get_num_params()/1e6,))
	...
	def forward(self, tok_emb, targets=None):
		...
		#     # loss = torch.sum((logits - targets)**2)
		# else:
